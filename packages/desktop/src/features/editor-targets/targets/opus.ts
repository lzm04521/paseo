...
type ResolvedOpusRuntime =
   ...
   * new front tab in the most recently active lister. `/acmd` (not `/cmd`) so that with no
   * active lister Opus opens a new window — `/cmd` targets the source lister and does
...
export async function tryLaunchOpus(
  ...
  if (resolved.kind === "runtime") {
    await runtime.spawnDetached({
      ...
      args: ["/acmd", "Go", targetPath, "NEWTAB=tofront"],
    });
    return true;
  }
  await runtime.spawnDetached({
