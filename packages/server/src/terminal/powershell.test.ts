...
import { BUILTIN_POWERSHELL_COMMAND, resolveBuiltinPowerShellCommand } from "./powershell.js";
...
describe("resolveBuiltinPowerShellCommand", () => {
  ...
  it("uses the PowerShell 7 default install dir when pwsh is not on PATH", async () => {
    await expect(harness({ files: ["C:\\Program Files\\PowerShell\\7\\pwsh.exe"] })).resolves.toBe(
