...
从 [Releases](https://github.com/lzm04521/paseo/releases/latest) 下载 `Paseo-Setup-<版本>-x64.exe`（如 `Paseo-Setup-0.7.2-local.4-x64.exe`）安装。仅支持 Windows x64。
...
- **资源管理器侧栏增强设置**：上游 v0.6.0 起提供文件 + 更改双树、可拖宽的资源管理器侧栏（设置 → 布局）。本仓库在「设置 → 布局 → 资源管理器侧栏」增设四个选项：
  - **打开工作区时自动展开**（默认关）：打开 workspace 时若侧栏处于关闭状态则自动展开；
  - **自动展开定位标签**（文件 / 变更，默认文件）：自动展开时定位到的树；
  - **侧栏打开文件方式**（当前选项卡（预览）/ 新选项卡，默认当前选项卡（预览））：从侧栏文件树点击文件时，「预览」沿用上游行为——未修改的文件选项卡被连续点击复用；「新选项卡」为每个文件单独打开选项卡，已打开的文件再次点击则激活原选项卡；
...
- 版本号 = 对应上游版本 + `-local.N`（如 `0.7.2-local.4`），同名 tag 触发 GitHub Actions 在 windows-latest 上构建 NSIS 安装包并发布。
