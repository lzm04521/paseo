...
- **「新建 workspace」页文件导航**：新建 workspace 时，右侧显示当前所选项目的文件导航（文件树），切换项目自动刷新；点击文件在面板内预览（代码高亮 / Markdown / 图片，带返回按钮，右键菜单仍可外部编辑器打开）。标题栏按钮 / Ctrl(Cmd)+E 开关，与工作区侧边栏同键位，开合状态跨会话记忆；拖动面板左缘调宽度（220–720px，同样记忆）。
