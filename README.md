...
- **文件 / 更改侧边栏（上游 v0.6.0 恢复）**：上游 v0.6.0 恢复了专用资源管理器侧边栏（文件 + 更改，默认常驻、可拖宽），并新增按操作细分的「打开位置」设置（设置 → 布局）；fork 此前自研的「文件导航」侧边栏面板与「侧边栏」外观设置（默认打开 / 占比 / 文件打开位置 / 自动打开）已于 0.6.0-local.1 起移除，直接采用上游实现。fork 另在「设置 → 布局 → 资源管理器侧栏」增设三个设置：「打开工作区时自动展开」开关（默认关）——打开 workspace 时若侧栏处于关闭状态则自动展开；「自动展开定位标签」（文件 / 变更，默认文件）——自动展开时定位到的树；「默认打开宽度」（10%–50%，默认 25%）——侧栏无记忆宽度时占工作区宽度的比例，拖动调整后按工作区记忆（均仅桌面宽布局生效，不覆盖紧凑窗口的悬浮侧栏）。
