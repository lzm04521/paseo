...
从 [Releases](https://github.com/lzm04521/paseo/releases/latest) 下载 `Paseo-Setup-<版本>-x64.exe`（如 `Paseo-Setup-0.7.2-local.6-x64.exe`）安装。仅支持 Windows x64。
...
- 版本号 = 对应上游版本 + `-local.N`（如 `0.7.2-local.6`），同名 tag 触发 GitHub Actions 在 windows-latest 上构建 NSIS 安装包并发布。
