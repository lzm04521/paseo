...
- **「新建 workspace」页文件导航**：新建 workspace 时，右侧显示当前所选项目的文件导航（文件树），切换项目自动刷新；点击文件用外部编辑器（无则文件管理器）打开。标题栏按钮 / Ctrl(Cmd)+E 开关，与工作区侧边栏同键位，开合状态跨会话记忆；拖动面板左缘调宽度（220–720px，同样记忆）。
