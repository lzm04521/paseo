...
> **已核对 local/v0.9.2（截至 2026-09-26，HEAD = port `a31db1b4a8` + README 还原 `c2d2f222f3` + bump `93711d1017` + README 示例 `6a5cd6be04`，base = v0.9.2 `c67b7158b4`；merge 快速合并，冲突 19 文件 = 14 版本固定集(取上游) + ci.yml UD(保持删除) + 4 代码手解 + 2 处 auto-merge 语义适配）**：上游 v0.9.2 为中版本（172 文件 +9.1k/-1.2k，修复为主：#5190 Add Project 目录扫描降本、**#5170 watcher 不可观察仓库占 CPU（降级轮询 5s×2^n 指数退避上限 60s）**、#5227 未挂载磁盘、#5253 plugin 改在 agent 全关后停、#5337/#5315 config.json 报错命名/UTF-8 BOM、rewind 三件、website sponsor rework）。**首次真功能碰撞并丢弃本地实现**：本地 stagger/backoff 降级轮询体系（`computeDegradedPollIntervalMs` 60s/120s+jitter、`fallbackIdlePolls`、`degradedPollRandom`）与上游 #5170 动机相同实现不同——按"上游已实现→丢弃"原则全取上游结构；**保留** forge PR 轮询降级三件（`classifyForgePollFailure`/`FORGE_POLL_DEGRADED_AFTER_CONSECUTIVE_ERRORS`/backoff cap 900_000）+ 本地 F5 字段（fetchTimer/consecutiveFetchErrors，与上游 intervalId 并存）。**2 处 auto-merge 语义适配**：① directory-suggestions.ts——上游 isNewlyGitIgnored 非 symlink 分支直查 `gitIgnoredPaths.has()` 绕过本地 B4 override 挂载点，在其非 symlink 分支前补 `isGitIgnoreOverridePath` 检查（保留上游 O(1) 优化）；② RepoGitTarget 接口 auto-merge 丢上游新字段 `intervalId`（typecheck TS2353 抓到补回）。agent.ts 8 处冲突全"上游新增+本地新增"双保留（rewindSdk/typedSlashCommandIndex ↔ customProvider/getDaemonConfig/downgrade）；bootstrap.ts stop() 删本地旧位置 stopAllPlugins（上游 #5253 挪后，保留会双重调用）。**新坑**：① merge 方向：固定集"取上游"= `--ours`（ours=新 tag 基底）；`--theirs` 误写入 index 后无法再用 `--ours` 恢复，须 `git checkout HEAD -- <files>` 重取；② auto-merge 过 ≠ 语义对（override 被绕过靠本地测试抓到、接口字段丢靠 typecheck 抓到）——merge 后必跑本地功能测试 + typecheck 双门禁；③ 本地测试的时序行/注释会混进上游同名用例（72_000 vs 上游 5_000/7_000/15_000 共 6 处），逐处对照上游版修；④ cli 报 `PidLockInfo.serverId` TS2339 时 `build:protocol` 不够——PidLockInfo 在 **server** 包，须 `build:server`。验证全过：directory-suggestions 31+9skip、i18n 36/36（首跑 1 fail 疑 formatChangelogDate locale 既有 flaky）、observation 54/54、主测试 62/62、agent 87/87（上游 +5 rewind）、primitive 55/55、typecheck 0 错、lint 0 error。详见 `handoff/20260926-handoff-port-to-v0.9.2.md`。
> **已核对 local/v0.9.1（截至 2026-09-23，HEAD = port `5a5eb3070f` + bump `9e27cb5b29` + README 版本示例，base = v0.9.1 `8186585201`；merge 快速合并，冲突 13 文件 = 全部版本固定集(取上游)，零手解）**：上游 v0.9.1 为小补丁版（29 文件 +490/-91：**Opus 5.5 进 Claude 目录并设默认模型**（1M 上下文+Fast Mode）#5200（model-manifest.ts + agent.test.ts 默认断言 `claude-opus-5-5`，含 minor 版本号模型 composer 回退修复）、#5189 app 重连 daemon 后 sidebar 只剩最近变更会话的修复（directory-sync/agent-replica.ts）、#5174 Import session 覆盖全部 Codex 会话（原先只 100 条）、heap snapshots 移出 .gitignore、lockfile/Nix hash 修正）。与本地补丁**零功能交集——全清单保留（连续第三版零交集）**。唯一交集 agent.test.ts 双侧 hunk 相距 2800 行（上游 L476 断言 / 本地 L3278 追加 fetchCatalog describe）auto-merge 直接过，测试 82/82。desktop-packages.yml 删除经 merge 自动带入（base 有→theirs 删→ours 未改=删），无需手 rm。**新坑**：① 起分支后的 README 还原（checkout main -- README.md）必须先 commit 再 merge，否则暂存改动挡 merge 报 overwritten；commit 后 README 双侧 fork 版自动合并成功；② 文件级交集 ≠ 冲突——先 `git diff <tag>..<branch> -- <file> | grep "^@@"` 比对双侧 hunk 行号再定策略；③ 相邻小版本浅克隆 `--deepen=100` 即连通（大版本才需 500）；④ main 更新本次零 UD workflow 冲突（上游未动 .github），两个 fork-mgmt cherry-pick（workflows 恢复 + desktop-packages 删除）直接落地。验证全过：i18n 36/36、agent.test.ts 82/82、typecheck 0 错、app lint 0 error/391 warning 基线一致。详见 `handoff/20260923-handoff-port-to-v0.9.1.md`。
> **已核对 local/v0.9.0（截至 2026-09-22，HEAD `ae7feaa7d3`（port `eb07edfa91` + bump `2b8b252774` + README `779a017f01` + workflow 清理），base = v0.9.0 `7f7e60bcbb`；merge 快速合并，冲突 25 文件 = 14 版本固定集(取上游) + 4 workflow UD(保持删除) + README(取 theirs) + 6 代码手解）**：上游 v0.9.0 为大版本（988 文件 +49.5k/-21k：Find 三处 #4589/#4650/#4765（file pane/terminal/chat）、**插件 npm 安装+`paseo plugin update` #4975（plugin schema 从 messages.ts 拆到新文件 plugin-config.ts，AgentSkillSelection 挪 agent-profile.ts）**、daemon 配置持久化 #4575、#4844 关最后 content tab→留 New launcher、#4838 daemon heap 修复（DaemonSession 新增 getObservationMetrics）、**#4973 new-workspace composer 增长修复（KeyboardTranslateView 与 contentStyle/getContentStyle 整体删除，容器改回 `styles.content`）**）。与本地补丁**零功能交集——全清单保留**（C 最后 agent tab 保护与 #4844 正交：本地拦截"关光"、上游管关光后的展示；#4575 未动 daemon-config-store 白名单注入链）。**手解模式全是"上游新增+本地新增"双保留**：① messages.ts——本地 schema（fileSearch/idleAutoRestart）留原位，plugin 相关定义丢弃（import 区已自动切 plugin-config.js/agent-profile.js）；② daemon-session.ts——getObservationMetrics + 本地 idle 时钟三处（接口/字段/构造）双保留；③ terminal-session-controller.ts——上游回调签名加 `source` 参数 + 删 `terminalSizeOwner`，本地 `resolveTerminalCommand` 保留、签名跟上游；④ session.ts——两处构造同②③模式；⑤ packaged-web-daemon.ts——上游 `PASEO_` env 过滤 + 本地 HOME 注释并存；⑥ new-workspace-screen.tsx——上游 `styles.content` 容器 + 本地 fileNav：`<ReanimatedAnimated.View style={[styles.content, contentNavPadStyle]}>` 外层、`{fileNavPanelElement}` 保留、KeyboardTranslateView 闭合删除，**本地旧容器的死 import（RNStyleSheet/HEADER_INNER_HEIGHT/MAX_CONTENT_WIDTH）要清**。zh-CN：lifecycle 段上游整段 `en.` fallback → 补译 13 条；plan cards 三 key 上游自带中文；noClipboardImage 取上游新长文案。lint 仅 1 处：provider-snapshot-manager.test.ts 解构遮蔽（`find(({entry})=>...)` 外层同名）改 `find((record)=>record.entry...)`。**新坑**：① 浅克隆 merge-base deepen=30 不够（tag 历史变深），`--deepen=500` 才连通（v0.9.0 距 v0.8.0 约 600+ commit）；② 上游 v0.9.0 新增 `.github/workflows/desktop-packages.yml`（**main push 触发**的打包冒烟），merge 不冲突自动带入——发版分支和 main 两处都要手动 `git rm`（旧 fork-mgmt cherry-pick 删除清单不含它）；③ cherry-pick fork-mgmt 再现 4 个 UD workflow 冲突（v0.9.0 改过 ci/desktop-release/docker/nix），照旧 git rm 保持删除。详见 `handoff/20260922-handoff-port-to-v0.9.0.md`。
> **已核对 local/v0.8.0（截至 2026-09-10，HEAD `9dcacad7b`（port `358ac6e4d` + bump `9f10bbe9a` + README），base = v0.8.0 `b8e24677e`；merge 快速合并，冲突 26 文件 = 13 版本固定集 + 4 workflow UD(保持删除) + zh-CN 2 处 + 6 代码手解）**：上游 v0.8.0 为大版本（905 文件 +58k/-11k：plugin 体系大扩——plugin custom providers #4314 / header buttons+composer pills #4577 / 插件 client-server 入口分离 #4347；**#4216 Home 按钮→Import session（en footer home key 已删，zh-CN 同步删）**；sidebar 重排 #4203；#4332 isAvailable 加 catalogOptions；#4322/#4201 updater 改动未动 configure channel——D4 无冲突）。与本地补丁**零功能交集——全清单保留**：#4314 是 plugin 注册 provider 类型（provider-config 的 extends/label 校验），与本地 fetchModels settings 手动表单不重叠（grep 上游 providers-section 无自定义表单证据）。**上游 API 变化适配**：① SettingsSection 移到 `@/components/settings/headings/settings-section`（旧 screens 路径已删）+ 新 `@/components/settings` 聚合组件体系（SettingsCard/Row/Switch/Select/Input/Action）——layout-section 整文件重写（本地 4 个 explorer sidebar 设置换 SettingsSwitch/SettingsSelect；**SettingsSelect 泛型只收 string**，number 宽度用 String/Number 转换）；② WorkspaceTabLaunchItem 新增必填 `toggleTarget`（本地 powershell item 补 null）；③ `getSnapshot()` 返回 `{cwd, records}`（本地 availability 超时测试改 `.records.find(({entry})=>...)?.entry`）；④ new-workspace composer 容器→KeyboardTranslateView（内层闭合取上游、外层保本地 Reanimated+fileNavPanel）。**lint/complexity 适配（merge 后本地引入 4 errors，上游原版 0）**：内联对象 prop → useMemo 提取；complexity 27→20 三连抽——`useNewWorkspaceFileNav` hook（布尔 && 链/`??`/手势全套挪进 hook）+ `NewWorkspaceFileNavPanel` 组件（preview/tree 分支）+ 渲染三元挪 useMemo 回调（**回调独立作用域不计组件 complexity**）+ FileNavToggleIcon。**新坑**：① 解完冲突先 `grep -rn "^<<<<<<<|^>>>>>>>"` 再 git add（本次一处残留标记随 commit 进入，amend 救回）；② dist 陈旧三连 build:protocol→build:client→build:server 才全过；③ cherry-pick fork-mgmt 再现 4 个 UD workflow 冲突（v0.8.0 改过这 4 个文件），git rm 保持删除；④ zh-CN 残留检查脚本放 `.tmp/` 跑完即删，tsx named import 失败用 dynamic import。zh-CN 补译 2 条（importSession.chooseHostTitle「从主机导入」+ settings.sections.layout「布局」既有漏译顺手修）。详见 `handoff/20260910-handoff-port-to-v0.8.0.md`。
> **已核对 local/v0.7.2（截至 2026-09-02，HEAD `a30c04f36`（port `fff1401d7` + bump `186fffbbf` + README），base = v0.7.2 `9400a49af`；merge 快速合并，冲突 14 文件 = 13 版本固定集 + workspace-git-service.ts 手解）**：上游 v0.7.1+v0.7.2 合计 165 文件 +5k/-1.5k（Fable 5.1 模型 / pi steering #3752 / Git 插件 build 命令 #4158 / diff 卡死修复 #4174 / **Windows daemon 优雅关闭 #4168** / Explorer rename 防覆盖 #4171 等），与本地补丁**零功能交集——全清单保留**（#4168 改 daemon-worker/supervisor/daemon-client 与本地 G watchdog 文件集不重叠；file-actions-menu/file-explorer-pane 上游零改动）。唯一手解：performRepoFetch 上游重构内层 try/catch + applyRepoFetchResult helper，本地 F5 catch（consecutiveFetchErrors+logFetchFailure）塞回上游结构（helper 的 `!result` 分支仅 `if (result)` 计数，与 catch 计数不重复）。i18n 上游仅 +1 key `agent.messageCapped`（自带 zh-CN 翻译）auto-merge 直接过。**新坑**：① protocol 新增子路径导出 `@getpaseo/protocol/plugin-source-reference`，server 测试报 Cannot find package = dist 陈旧 → `npm run build:protocol`（踩坑 #10 再一变体）；② 踩坑 #11 实例：跑完 vitest 后 cwd 停 packages/app，`git diff <t1> <t2> -- packages/app/...` 相对 pathspec 匹配不到**静默输出空**（diff/stat/raw 全空），一度误判上游 i18n 零改动；`git rev-parse <tag>:<file>` 不受 cwd 影响可暴露矛盾。cherry-pick fork-mgmt 到 v0.7.2 基底零冲突自动落地（0.7.0 的 ci.yml modify/delete 未复现）。详见 `handoff/20260902-handoff-port-to-v0.7.2.md`。
> **已核对 local/v0.7.0（截至 2026-09-01，HEAD `7da4b5d2a`（port `17488bbc8` + bump `66db107fe` + README），base = v0.7.0 `c56638ea8`；merge 快速合并，冲突 20 文件 = 13 版本固定集 + ci.yml modify/delete(保持删除) + file-explorer-pane/use-settings storage/layout-section/provider-registry/provider-snapshot-manager/bootstrap 手解）**：上游 v0.7.0 为大版本（661 文件 +39k/-8.9k：SSH 远程 daemon #3989 / 语义化 session 权限 #3981 / Command Center 工作区操作 #3013 / 插件 Git 安装 #3920 / **#3615 子文件夹「在 {首选编辑器} 打开」**/license 改 Apache-2.0）。唯一功能交集 = #3615：上游新建 `open-in-editor/` 目录（button/planner/directory，走 preferred editor + desktop-open-targets）覆盖 B3 文件夹场景 → **B3 文件夹部分丢弃**（menu 项 `fileKind === "file"` 限定 + pane handler 删 directory 分支）、**文件部分保留**（上游无文件右键入口）、B2 sidebar 保留。F1~F5 全保留（上游 #3945 event-loop 调度 / #3825 PR 批量轮询与本地正交）。**适配**：① i18n 8 语种修上游 openInSidePane 重命名（explorerChanges+changesLinks→diffs、新增 destinations.explorer；zh-CN 翻译其余英文 fallback，行级 python 批量）；② 上游 refresh 超时函数改名 `providerRefreshDeadline`，本地 `resolveRefreshTimeoutMs` 淘汰、`resolveAvailabilityTimeoutMs` 保留；③ bootstrap 重构为 `createAgentProviderRuntime` 包装，本地 `getDaemonConfig` 注入改走其 snapshotManager 透传字段；④ lint：file-actions-menu 本地项抽 useMemo（对称上游 openInEditorAction）、bootstrap watchdog 组装抽 `startDaemonIdleRestartWatchdog` helper 消 complexity 21。**新坑**：① `git cherry-pick` **不支持 `--no-verify`**（usage 报错 exit 129）——main 恢复用 plain cherry-pick，解冲突后手动 `git commit --no-verify`（`--continue` 会跑 lefthook 且静默失败、CHERRY_PICK_HEAD 残留）；② 根口径 react-perf/perf 为 error 但 **app 口径（expo lint）才是验收口径**（0 error 达标），本地 layout-section 既有 react-perf 违规与上游自身违规一样非阻塞；③ plugin-examples 引用新 API 报 TS2305 = `@getpaseo/plugin` dist 陈旧 → `npm run build:plugin`（踩坑 #10 再一变体）。详见 `handoff/20260901-handoff-port-to-v0.7.0.md`。
...
3. **右侧文件浏览器菜单增强**（`file-actions-menu.tsx` + `file-explorer-pane.tsx`；**v0.4.0 起部分由上游实现，见下**）：
   ...
   - **在 VSCode 打开（v0.7.0 起仅文件）**：本地独有保留（上游无文件右键编辑器入口）。v0.4.0 上游重写菜单结构（props 链四层：`renderTreeRow` → `TreeRowDispatcher` → `TreeRowItem` → `FileActionsContextMenuContent`），重插点：menu 加 `onOpenInVSCode` prop + spec（v0.7.0 起抽 `openInVSCodeAction` useMemo，对称上游 openInEditorAction，且 `fileKind === "file"` 限定）+ deps；pane 每层加 `onOpenInVSCodeEntry` prop、`TreeRowItem` 加 `handleOpenInVSCode`、主组件 `useOpenInVSCode()` + `handleOpenInVSCodeEntry`（v0.7.0 起仅文件：workspace root + filePath；**文件夹分支已丢**——上游 #3615 用 `useOpenDirectoryInEditor` 走 preferred editor，菜单项 open-in-editor 仅 directory 显示）+ renderTreeRow deps
...
6. **侧栏文件打开方式设置**（0.7.2-local.2 起，commit `38ee0a9f4`）：设置 `explorerFileOpenMode: "preview" | "tab"`（默认 `preview` 保持上游行为），UI 在 Settings → Layout → Explorer sidebar 分区。上游 0.6.0 `c914bcb44`（#3826）的 `openPreferredWorkspacePreview`（`packages/app/src/workspace-tabs/open-beside.ts`）让侧栏树单击文件复用目标 pane 未 modified 的 file tab（`canReplacePreview` 查 `getPanelInstanceAttributes().modified`）→ 连续点击多文件只占一个 tab；`"tab"` 模式在 `workspace-screen.tsx` 的 `onOpenPreferredTarget` 分流改走 `openPreferredWorkspaceTarget`（reveal 语义：同文件已开则激活、否则新 tab），条件 `explorerFileOpenMode === "tab" && target.kind === "file"`（记得 useCallback 依赖数组补 `explorerFileOpenMode`）。设置存储走 storage.ts `z.enum(["preview","tab"]).catch(DEFAULT)` + `ExplorerFileOpenMode` 类型从 use-settings/index.ts 导出；i18n 9 语种 `settings.layout.explorerSidebar.fileOpenModes.*` 四键。跟进新版时复查：上游若出原生 preview-tab 开关则评估丢弃。
